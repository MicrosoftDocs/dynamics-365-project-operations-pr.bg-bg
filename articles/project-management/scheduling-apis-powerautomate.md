...
description: Тази статия предоставя примерен поток, който използва програмните интерфейси на project schedule application (API).
author: ruhercul
ms.date: 01/26/2022
ms.topic: article
...
Тази статия се описва примерен поток, който показва как да създадете пълен план на проект с помощта Microsoft Power Automate на, как да създадете набор операция и как да актуализирате обект. Примерът показва как да създадете проект, член на екипа на проекта, Operation Sets, проектни задачи и задания на ресурси. Тази статия също така обяснява как да актуализирате обект и да изпълните набор операция.

По-долу е пълен списък на стъпките, които са документирани в примерния поток в тази статия:
...
Тази статия предполага, че имате основни познания за Dataverse платформата, потоците в облака и програмния интерфейс на project Schedule Application Interface (API). За повече информация вижте [раздела препратки](#references) по-късно в тази статия.
...
4. В Изберете **как да задействате този списък с потоци** изберете **Power Apps**. Когато създавате Power Apps спусък, логиката е до вас като автор. В тази статия оставете входните параметри празни за целите на тестването.
