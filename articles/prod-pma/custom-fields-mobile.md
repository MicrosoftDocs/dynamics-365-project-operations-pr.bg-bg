...
description: Тази статия предоставя общи модели за използване на разширения за внедряване на персонализирани полета.
author: Yowelle
ms.date: 05/29/2019
ms.topic: article
ms.prod: ''
...
Тази статия предоставя общи модели за използване на разширения за внедряване на персонализирани полета. Обхванати са следните статии:
...
Тази статия е предназначена за разработчици, които интегрират персонализираните си полета в мобилното Microsoft Dynamics 365 Project Timesheet приложение, което е достъпно за Apple iOS и Google Android. Предполага се, че читателите са запознати с X ++ разработката и функционалността на работния график на проекта.
...
- Ако свойството **stringOptions** е предоставено на обекта **TSTimesheetCustomField**, тези елементи от списъка са единствените стойности, които потребителите могат да изберат с помощта на бутоните за опции (радиобутони).
    ...
    В този случай полето на низа може да действа като стойност на изброяване с цел въвеждане от потребителя. За да запишете стойността в базата данни като енум, ръчно нанесете стойността на низа обратно към стойността на enum, преди да запишете в базата данни с помощта на командната верига (вижте раздела "Използване на командната верига в класа TSTimesheetEntryService, за да запишете запис в времеви лист от приложението обратно в базата данни" по-късно в тази статия за пример).
...
Това свойство е приложимо само когато **fieldBaseType** е зададено на **Низ**. Ако е зададено **stringOptions**, низовите стойности, които са достъпни за избор чрез бутони за опции (радиобутони), се определят от низовете в списъка. Ако не са предоставени низове, записът със свободен текст в полето за низове е разрешен (вижте раздела "Използване на командна верига в класа TSTimesheetEntryService, за да запишете запис в времеви лист от приложението обратно в базата данни" по-късно в тази статия за пример).
